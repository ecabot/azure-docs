...
2. In **Operations**, click **Disaster recovery (preview)**.
3. In **Configure disaster recovery** > **Target region** select the target region to which you'll replicate.
4. For this Quickstart, accept the defaults for Target settings, Storage settings, and Replication settings.
...
When replication is disabled, the VM will stop replicating to the target region.  The source replication settings are automatically removed, and Site Recovery billing for the VM also stops.
